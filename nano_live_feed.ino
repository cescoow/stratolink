...
void setup()
{
  ...
  //LoRa.setPins(10, 9, 2); //LoRa.setPins(ss, reset, dio0) Mega
  LoRa.setPins(7, 9, 2); //LoRa.setPins(ss, reset, dio0) Nano
...
void read_data_sd(){
  ...
  if (myFile) {
    ...
    while (myFile.available()) {
      ...
      delay(10);
      LoRa.beginPacket();
      //LoRa.write(byte_array);
      for (int i=0; i <= 199; i++){
        LoRa.write(byte_array[i]);
        //Serial.write(byte_array[i]);
        //data_lora.concat(byte_array[i]);
      }
